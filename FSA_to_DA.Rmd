...
To get the Census 2016 variables at the FSA level, we take the weighted mean, using population. FSAs var in population, which is why we use the weighted mean. 
...
PredictedValues %>%
  filter(Split == "GTA") %>% 
  mutate(CD_UID = case_when(stringi::stri_sub(CFSAUID, 1, 1) %in% c("L4", "L5") ~ "3521",
                            TRUE ~ CD_UID)) %>%
  as_tibble() %>% 
